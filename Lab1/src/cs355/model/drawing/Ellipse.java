...
public class Ellipse extends Shape {
	...
	public boolean pointInShape(Point2D.Double pt, double tolerance) {
		...
		double distance = (Math.pow(objectCoordinate.getX(), 2)/ Math.pow(width/2,2) + (Math.pow( objectCoordinate.getY(), 2)/Math.pow(height/2, 2))) ;
		if (distance <= 1){
			System.out.println("Ellipse seleceted");
			return true;
		}
		return  false;
	}
